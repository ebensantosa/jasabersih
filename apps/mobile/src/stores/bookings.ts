    ...
    bookingsSyncPromise = (async () => {
      ...
      const serverMapped: Booking[] = items.map((s) => {
        ...
        return existing ? {
          ...
          startedAt: (() => { const r = (s as any).startedAt ?? (s as any).started_at; return r != null ? safeTimestamp(r) : existing.startedAt; })(),
          pauseStartedAt: (() => { const r = (s as any).pauseStartedAt ?? (s as any).pause_started_at; if ('pauseStartedAt' in (s as any) || 'pause_started_at' in (s as any)) return r ? safeTimestamp(r) : undefined; return existing.pauseStartedAt; })(),
