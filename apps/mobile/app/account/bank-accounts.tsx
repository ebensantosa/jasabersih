...
function CustomerBankAccounts() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const r = await api.get('/customer/bank-accounts');
  ...
  async function setDefault(id: string) {
    try {
      await api.patch(`/customer/bank-accounts/${id}/set-default`);
  ...
  async function confirmDelete() {
    ...
    setDeleting(true);
    try {
      await api.delete(`/customer/bank-accounts/${deleteTarget.id}`);
      ...
      <View className="flex-1 bg-ink-50">
        <SafeAreaView edges={['top']} className="bg-white">
          <View className="flex-row items-center gap-2 px-2 py-2">
            ...
            <Text className="ml-1 text-base font-bold text-ink-900">Rekening Pencairan</Text>
          </View>
        </SafeAreaView>

        <ScrollView contentContainerStyle={{ padding: 16, gap: 12 }}>
          <View className="rounded-xl border border-blue-200 bg-blue-50 p-3 flex-row gap-2">
            <BadgeCheck color="#1D4ED8" size={18} />
            <Text className="flex-1 text-xs leading-5 text-blue-900">
              Tambah rekening bank atau e-wallet atas nama kamu. Sistem akan verifikasi otomatis dan hanya tujuan verified yang bisa dipakai untuk tarik saldo referral.
            </Text>
          </View>
          ...
          {loading ? (
            ...
            accounts.map((acc) => {
              ...
              return (
                <View key={acc.id} className="rounded-xl border border-ink-100 bg-white p-4">
                  <View className="flex-row items-center gap-3">
                    ...
                    <View className="flex-1">
                      ...
                      <Text className="mt-0.5 text-xs text-ink-500">{acc.accountNumber}</Text>
                    </View>
                    {acc.isVerified && (
                  ...
                  <View className="mt-3 flex-row gap-2">
                    {!acc.isDefault ? (
                      <Pressable onPress={() => setDefault(acc.id)} className="flex-row items-center gap-1 rounded-lg bg-ink-100 px-3 py-1.5">
                        <Star color="#0F172A" size={14} />
                        <Text className="text-xs font-semibold text-ink-900">Jadikan Utama</Text>
                      </Pressable>
                    ) : (
                      <View className="flex-row items-center gap-1 rounded-lg bg-amber-100 px-3 py-1.5">
                        <Star color="#D97706" size={14} fill="#D97706" />
                        <Text className="text-xs font-semibold text-amber-700">Default</Text>
                      </View>
                    )}
                    <Pressable onPress={() => setDeleteTarget(acc)} className="ml-auto flex-row items-center gap-1 rounded-lg bg-rose-50 px-3 py-1.5">
          ...
          <Pressable onPress={() => setAddModal(true)} className="mt-2 flex-row items-center justify-center gap-2 rounded-xl bg-blue-600 py-4">
            <Plus color="white" size={20} />
            <Text className="text-base font-bold text-white">Tambah Tujuan Pencairan</Text>
          </Pressable>
        </ScrollView>
      </View>

      <AddBankModal visible={addModal} onClose={() => setAddModal(false)} onDone={() => { setAddModal(false); void load(); }} />
      <DeleteConfirmModal target={deleteTarget} deleting={deleting} onCancel={() => setDeleteTarget(null)} onConfirm={confirmDelete} />
    </>
  );
}
...
function DeleteConfirmModal({
  ...
  return (
    <Modal transparent statusBarTranslucent visible animationType="fade" onRequestClose={onCancel}>
      <Pressable onPress={onCancel} style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
        <Pressable onPress={(e) => e.stopPropagation()} className="w-full max-w-sm rounded-2xl bg-white p-5">
          <View className="items-center">
            ...
            <Text className="mt-3 text-center text-base font-extrabold text-ink-900">Hapus {isEwallet ? 'E-Wallet' : 'Rekening'}?</Text>
            <Text className="mt-1.5 text-center text-[12px] font-medium text-ink-600">
          ...
          <View style={{ marginTop: 20, flexDirection: 'row', gap: 8 }}>
            <Pressable onPress={onCancel} disabled={deleting} style={{ flex: 1, alignItems: 'center', borderRadius: 12, borderWidth: 1, borderColor: '#CBD5E1', paddingVertical: 12, backgroundColor: '#FFFFFF' }}>
              <Text style={{ fontWeight: '600', fontSize: 14, color: '#334155' }}>Batal</Text>
            </Pressable>
            <Pressable onPress={onConfirm} disabled={deleting} style={{ flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, borderRadius: 12, paddingVertical: 12, backgroundColor: '#DC2626', opacity: deleting ? 0.6 : 1 }}>
              {deleting && <ActivityIndicator size="small" color="#FFFFFF" />}
              <Text style={{ fontWeight: '700', fontSize: 14, color: '#FFFFFF' }}>{deleting ? 'Menghapus...' : 'Ya, Hapus'}</Text>
            </Pressable>
          </View>
        </Pressable>
      </Pressable>
...
function AddBankModal({ visible, onClose, onDone }: { visible: boolean; onClose: () => void; onDone: () => void }) {
  ...
  async function submit() {
    ...
    setVerifying(true);
    try {
      const r = await api.post('/customer/bank-accounts', { bankCode, accountNumber });
      ...
      <Pressable onPress={onClose} className="flex-1 justify-end bg-black/50">
        <Pressable onPress={(e) => e.stopPropagation()} className="rounded-t-3xl bg-white p-5 pb-8">
          <View className="mb-4 flex-row items-center justify-between">
            <Text className="text-lg font-bold text-ink-900">Tambah Tujuan Pencairan</Text>
          ...
          <View className="mb-4 flex-row rounded-xl bg-ink-100 p-1">
            {([
              ...
              const Icon = t.icon;
              return (
                <Pressable key={t.key} onPress={() => onTypeChange(t.key)} className={`flex-1 flex-row items-center justify-center gap-1.5 rounded-lg py-2.5 ${active ? 'bg-white' : ''}`}>
                  <Icon color={active ? '#1D4ED8' : '#94A3B8'} size={14} strokeWidth={2.4} />
                  <Text className={`text-[13px] font-bold ${active ? 'text-brand-700' : 'text-ink-500'}`}>{t.label}</Text>
                </Pressable>
              );
            })}
          </View>

          <Text className="mb-2 text-sm font-semibold text-ink-700">{isEwallet ? 'Pilih E-Wallet' : 'Pilih Bank'}</Text>
          <View className="mb-4">
            <Dropdown
          ...
          <Text className="mb-2 text-sm font-semibold text-ink-700">{isEwallet ? 'Nomor HP Terdaftar' : 'Nomor Rekening'}</Text>
          <TextInput
            value={accountNumber}
            ...
            className="rounded-xl border border-ink-200 px-4 py-3 text-base"
          />
          ...
          <Pressable onPress={submit} disabled={verifying} className={`mt-5 flex-row items-center justify-center gap-2 rounded-xl py-4 ${verifying ? 'bg-blue-300' : 'bg-blue-600'}`}>
            {verifying && <ActivityIndicator color="white" />}
            <Text className="text-base font-bold text-white">{verifying ? 'Memverifikasi...' : 'Verifikasi & Simpan'}</Text>
          </Pressable>
        </Pressable>
      </Pressable>
    </Modal>
...
export default withAuth(CustomerBankAccounts, 'customer');
