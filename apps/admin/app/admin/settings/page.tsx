...
import { Plus, Shield, Banknote, Briefcase, Ban, Activity, Pencil, Trash2, Mail, Send } from 'lucide-react';
...
function EmailTab() {
  ...
  async function sendTest() {
    if (!testTo || !/^.+@.+\..+$/.test(testTo)) { toast.error('Email tujuan tidak valid.'); return; }
    setTesting(true);
    try {
    ...
    } catch (e: any) { toast.error(e?.message ?? 'Gagal kirim.'); }
    finally { setTesting(false); }
  }

  if (loading) return <div className="py-10 text-center text-sm text-slate-500">Memuat…</div>;

  return (
    <div className="max-w-2xl space-y-4">
      <div className="rounded-md border bg-blue-50 p-3 text-xs text-blue-900">
        <b>Resend setup:</b> daftar di <a className="underline" href="https://resend.com" target="_blank">resend.com</a>,
        verifikasi domain (jasabersih.com), bikin API key di <a className="underline" href="https://resend.com/api-keys" target="_blank">resend.com/api-keys</a>,
        lalu paste di sini. From address harus pakai email di domain yang udah verified.
      </div>
      <div className="rounded-md border bg-white p-4 space-y-3">
        <h2 className="text-base font-semibold">Konfigurasi Resend</h2>
        <div>
          ...
          <div className="flex gap-2">
            ...
            />
            <Button variant="secondary" onClick={() => setShowKey((v) => !v)}>{showKey ? 'Sembunyikan' : 'Tampilkan'}</Button>
          </div>
          <p className="mt-1 text-[11px] text-slate-500">Disimpan di app_config (DB) — bisa diubah tanpa restart server.</p>
        </div>
        <Input label="From Address" required value={fromAddress} onChange={setFromAddress} placeholder="noreply@jasabersih.com" helpText="Harus email di domain yang sudah verified di Resend." />
        <Input label="From Name" required value={fromName} onChange={setFromName} placeholder="JasaBersih" helpText="Nama yang muncul sebagai sender di inbox penerima." />
        <Button variant="primary" onClick={save} loading={saving}>Simpan Konfigurasi</Button>
      </div>

      <div className="rounded-md border bg-white p-4 space-y-3">
        <h2 className="text-base font-semibold">Test Send</h2>
        <p className="text-xs text-slate-500">Kirim email tes untuk verifikasi konfigurasi sudah benar.</p>
        <div className="flex gap-2">
          <input
            type="email"
            value={testTo}
          ...
          />
          <Button variant="primary" onClick={sendTest} loading={testing} icon={<Send size={14} />}>Kirim Tes</Button>
        </div>
      </div>

      <div className="rounded-md border bg-amber-50 p-3 text-xs text-amber-900">
        <b>Dipakai untuk:</b> OTP verifikasi pendaftaran (email lebih reliable daripada SMS),
        notifikasi booking, password reset, dan broadcast kalau dibutuhkan.
      </div>
    </div>
  );
}
