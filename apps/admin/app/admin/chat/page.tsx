...
export default function ChatAuditPage() {
  ...
  return (
    <div>
      ...
      <p className="text-sm text-slate-500">Monitor chat customer ↔ cleaner. PDP compliant — semua akses dicatat.</p>
    ...
    <div>
      <div className="mb-3 flex items-center gap-2">
        <div className="relative flex-1">
          ...
          <input value={q} onChange={(e) => setQ(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && load()} placeholder="Cari nama atau no HP customer/cleaner…" className="w-full rounded-md border border-slate-300 py-2 pl-9 pr-3 text-sm" />
        </div>
            ...
            <tbody>
              {list.map((b) => (
                <tr key={b.bookingId} className="border-t hover:bg-slate-50">
                  ...
                  <td className="px-4 py-2 text-xs text-slate-500">{b.lastMessageAt ? new Date(b.lastMessageAt).toLocaleString('id-ID') : '—'}</td>
...
function ThreadModal({ booking, onClose }: { booking: any; onClose: () => void }) {
  ...
  async function load() {
    if (!reason || reason.length < 3) return toast.error('Alasan akses wajib (min 3 karakter) — dicatat untuk PDP.');
    setBusy(true);
  ...
  return (
    <Modal title={`Chat — ${booking.customerName ?? '—'} ↔ ${booking.cleanerName ?? 'Cleaner'}`} open={true} onClose={onClose} size="xl">
      {!messages ? (
        <div className="space-y-4">
          <div className="rounded-md border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900">
            <ShieldAlert size={14} className="inline" /> <b>UU PDP</b> — Akses chat ini akan dicatat di <code>data_access_log</code>.
          </div>
        ...
        <div className="space-y-2">
          <div className="text-xs text-slate-500">Total: {messages.length} pesan · Akses tercatat: <b>{reason}</b></div>
...
function BlockedTab() {
  ...
  return (
    <div>
      <h2 className="mb-3 text-base font-semibold">Blocked Messages — Off-Platform Detection</h2>
      {loading ? (
        <div className="py-10 text-center text-sm text-slate-500">Memuat…</div>
      ) : list.length === 0 ? (
            ...
            <tbody>
              {list.map((m) => (
                <tr key={m.id} className="border-t align-top">
                  ...
                  <td className="px-4 py-2"><div className="font-medium">{m.senderName ?? '—'}</div><div className="text-xs text-slate-500">{m.senderPhone}</div>{m.senderStatus && m.senderStatus !== 'active' && <Badge variant="red">{m.senderStatus}</Badge>}</td>
                  ...
                  <td className="px-4 py-2 font-mono text-xs">{m.bookingId?.slice(0, 8)}…</td>
                </tr>
              ))}
            </tbody>
          </table>
...
function StatsTab() {
  ...
  if (!stats) return <div className="py-10 text-center text-sm text-slate-500">Memuat…</div>;
  ...
  return (
    <div>
      ...
      <h3 className="mb-2 mt-6 text-sm font-semibold">Block Reason — 30 hari terakhir</h3>
